...
# 서브프로세스 실행 함수
def run_subroutine(task_name, command, log_file_path, progress_queue, message_queue, index, total_tasks, task_start_event):
    # 작업 시작 이벤트 대기
...
class ArtifactExtractorApp:
    def __init__(self, root):
        ...
        # 현재 작업 레이블
        self.current_task_label = ttk.Label(self.main_frame, text="대기 중...")
        self.current_task_label.grid(row=1, column=0, columnspan=3, pady=(0, 10))

        # 진행률 바
        self.progress_bar = ttk.Progressbar(self.main_frame, orient="horizontal", mode="determinate")
        self.progress_bar.grid(row=2, column=0, columnspan=3, sticky='we', pady=(0, 10))
        self.progress_bar["maximum"] = 100

        # 진행률 퍼센트 레이블
        self.progress_percent_label = ttk.Label(self.main_frame, text="진행률: 0%")
        self.progress_percent_label.grid(row=3, column=0, pady=(0, 10))

        # 예상 남은 시간 레이블
        self.eta_label = ttk.Label(self.main_frame, text="예상 남은 시간: 계산 중...")
        self.eta_label.grid(row=3, column=1, pady=(0, 10))

        # 경과 시간 레이블
        self.elapsed_time_label = ttk.Label(self.main_frame, text="경과 시간: 0분 0초")
        self.elapsed_time_label.grid(row=3, column=2, pady=(0, 10))

        # 출력 창
        self.output_frame = ttk.Frame(self.main_frame)
        self.output_frame.grid(row=4, column=0, columnspan=3, sticky='nsew')
        self.output_frame.rowconfigure(0, weight=1)
        self.output_frame.columnconfigure(0, weight=1)

        self.output_text = tk.Text(self.output_frame, wrap='word', state=tk.DISABLED)
        ...
        # 시작 버튼
        self.start_button = ttk.Button(self.main_frame, text="추출 시작", command=self.start_extraction)
        self.start_button.grid(row=5, column=0, columnspan=3, pady=(10, 0))
        ...
        self.subroutines = [
            ('웹 아티팩트 처리', ['python', r'subroutine\web\web_complete.py']),
            ('프리패치 아티팩트 처리', ['python', r'subroutine\prefetch\prefetch_complete.py']),
            ('이벤트 로그 아티팩트 처리', ['python', r'subroutine\event_log\evt_complete.py']),
            ('MFTJ 아티팩트 처리', ['python', r'subroutine\MFTJ\mftJ_complete.py']),
            ('LNK 아티팩트 처리', ['python', r'subroutine\LNK\LNK_complete.py']),
            ('CSV 파일 결합', ['python', 'csv_totaler.py'])
        ]
        self.total_tasks = len(self.subroutines)
        self.progress_queue = queue.Queue()
        self.message_queue = queue.Queue()
        self.log_file_path = "process_logs.txt"
        self.elapsed_times = [0] * self.total_tasks  # 각 작업의 소요 시간 저장
    ...
    def process_queues(self):
        ...
        while not self.progress_queue.empty():
            ...
            # 진행률 업데이트는 update_progress_bar에서 처리
    ...
    def run_subroutines(self):
        ...
        futures = []
        for index, (task_name, command) in enumerate(self.subroutines):
        ...
        self.current_task_label.config(text="완료! combined_analysis.xlsx 파일을 확인하세요.")
        ...
        while self.progress_updater_running:
            with self.lock:
                ...
                if self.completed_tasks > 0:
                    ...
                    estimated_total_time = elapsed * self.total_tasks / (self.current_task_index + 1)
                progress = (elapsed / estimated_total_time) * 100
                if progress > 100:
                    progress = 100
