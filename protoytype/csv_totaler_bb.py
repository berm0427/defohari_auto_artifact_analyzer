...
def deduplicate_sus_files(sus_df):
    ...
    if missing_columns:
        print(f"Cannot deduplicate because the following columns are missing: {missing_columns}")
        # 필요한 경우 여기에서 예외를 발생시키거나 다른 방식으로 처리할 수 있습니다.
        return sus_df
    ...
    for key, file_info in paths.items():
        ...
        if file_path.endswith(".csv") and os.path.exists(file_path):
            try:
                ...
                continue  # CSV 읽기에 실패하면 다음 파일로 이동
            ...
            try:
                ...
                print(f"Error occurred while processing DataFrame {file_name}: {e}")
        else:
