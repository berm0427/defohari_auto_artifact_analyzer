...
def download_google_sheet_as_excel(sheet_id, json_credentials_path):
    ...
    if response.status_code == 200:
        with open(r"..\..\output\suspicious_artifact\LNK\suspicious_antiforensic_lnk_files.xlsx", "wb") as f:
            f.write(response.content)
        print("File downloaded successfully as 'suspicious_antiforensic_lnk_files.xlsx'.")
    else:
        print(f"Failed to download the file. Status code: {response.status_code}")

# Path to your CSV file
# csv_file_path = r'..\..\output\artifact\LNK\lnk_files.csv'
csv_file_path = r'..\..\output\suspicious_artifact\LNK\suspicious_antiforensic_lnk_files_ccno.csv'
