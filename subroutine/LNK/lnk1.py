...
# LNK 파일 분석
def parse_lnk_file(file_path, shell, username):
    try:
        ...
        updated_target_path = update_user_path(target_path, username)

        return {
            "file_path": updated_file_path,
            "target_file_path": updated_target_path if updated_target_path else None,
...
def analyze_lnk_files(directory, username):
        ...
        for root, _, files in os.walk(directory):
            for file in files:
                if file.lower().endswith('.lnk'):
                    ...
                    lnk_info = parse_lnk_file(file_path, shell, username)
                    if lnk_info:
...
if __name__ == "__main__":
    ...
    for user in users_data:  # 여러 사용자가 있을 수 있으므로 반복 처리
        ...
        # LNK 파일 분석
        lnk_data_list = analyze_lnk_files(lnk_directory, user)
