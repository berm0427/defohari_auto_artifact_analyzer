...
def get_lnk_files(image_path, offset, start_inode=None):
    ...
    try:
        ...
        # 수정된 패턴: 모든 접두사를 포괄
        pattern = re.compile(r'^[^:]*\s+(\d+-\d+-\d+):\s+(.*\.lnk)$', re.IGNORECASE)
...
def main():
    ...
    try:
        ...
        for line in result.stdout.splitlines():
            ...
            if match:
                ...
                print(f"찾은 사용자 디렉토리: {username}, inode: {inode}")  # 실시간 출력
    ...
    # 모든 사용자에 대해 'AppData\Roaming\Microsoft\Windows\Recent' 디렉토리 검색 및 .lnk 파일 추출
    extracted_data = []
    for user in user_dirs:
        ...
        for lnk in lnk_files:
            ...
            print(f"{file_path}의 시간 정보 추출 완료")  # 실시간 출력

    if not extracted_data:
        logging.warning("모든 사용자 디렉토리에서 .lnk 파일을 찾지 못했습니다.")
        print("모든 사용자 디렉토리에서 .lnk 파일을 찾지 못했습니다.")
        exit(0)

    # CSV로 저장
    csv_file = os.path.join(output_dir, 'lnk_files_times.csv')
    write_to_csv(extracted_data, csv_file)

    # XML로 저장
    xml_file = os.path.join(output_dir, 'lnk_files_times.xml')
    write_to_xml(extracted_data, xml_file)
