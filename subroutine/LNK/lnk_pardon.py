    ...
    try:
        ...
        with open(csv_file_path, mode='w', newline='', encoding='utf-8') as file:
            ...
            for lnk_file in root.findall('LnkFile'):
                # Safely accessing XML elements with default values
                file_path = (lnk_file.find('file_path').text 
                             if lnk_file.find('file_path') is not None else 'N/A')
                target_file_path = (lnk_file.find('target_file_path').text 
                                    if lnk_file.find('target_file_path') is not None else 'N/A')
                creation_time = (lnk_file.find('creation_time').text 
                                 if lnk_file.find('creation_time') is not None else 'N/A')
                access_time = (lnk_file.find('access_time').text 
                               if lnk_file.find('access_time') is not None else 'N/A')
                write_time = (lnk_file.find('write_time').text 
                              if lnk_file.find('write_time') is not None else 'N/A')
                usb_name = (lnk_file.find('usb_name').text 
                            if lnk_file.find('usb_name') is not None else 'N/A')
