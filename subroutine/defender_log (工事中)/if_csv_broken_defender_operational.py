...
def download_google_sheet_as_excel(sheet_id, json_credentials_path):
    ...
    if response.status_code == 200:
        with open(r"..\..\output\artifact\defender_log\defender_operational.xlsx", "wb") as f:
            f.write(response.content)
        print("File downloaded successfully as 'defender_operational.xlsx'.")
    else:
        print(f"Failed to download the file. Status code: {response.status_code}")

# Path to your CSV file
csv_file_path = r'..\..\output\artifact\defender_log\Microsoft-Windows-Windows Defender%4Operational.csv'
