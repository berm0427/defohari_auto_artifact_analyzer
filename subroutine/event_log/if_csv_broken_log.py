...
def download_google_sheet_as_excel(sheet_id, json_credentials_path):
    ...
    if response.status_code == 200:
        with open(r"..\..\output\artifact\event_log\evt_logs.xlsx", "wb") as f:
            f.write(response.content)
        print("File downloaded successfully as 'evt_logs.xlsx'.")
    else:
        print(f"Failed to download the file. Status code: {response.status_code}")

# Path to your CSV file
# csv_file_path = r'..\..\output\artifact\LNK\lnk_files.csv'
csv_file_path = r'..\..\output\artifact\event_log\event_logs.csv'
