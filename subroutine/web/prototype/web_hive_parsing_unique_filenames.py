...
# 파일을 추출하는 함수
def extract_file(image_path, offset, inode, output_dir, file_name, inode_suffix):
    """icat 도구로 파일을 추출하는 함수"""
    try:
        os.makedirs(output_dir, exist_ok=True)
        output_file = os.path.join(output_dir, f"{file_name}_{inode_suffix}")
...
def extract_hives(image_path, output_dir):
    ...
    # SAM 및 SAM.LOG* 파일들 추출 (파일 이름에 inode 번호 추가)
    for inode in sam_inodes:
        extract_file(image_path, partition_offset, inode, output_dir, "SAM", inode)
    for inode in sam_log_inodes:
        extract_file(image_path, partition_offset, inode, output_dir, "SAM.LOG", inode)

    # SYSTEM 및 SYSTEM.LOG* 파일들 추출 (파일 이름에 inode 번호 추가)
    for inode in system_inodes:
        extract_file(image_path, partition_offset, inode, output_dir, "SYSTEM", inode)
    for inode in system_log_inodes:
        extract_file(image_path, partition_offset, inode, output_dir, "SYSTEM.LOG", inode)
